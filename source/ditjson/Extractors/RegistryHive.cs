...
namespace ditjson.Extractors
{
    ...
    internal sealed class RegistryHive : IDisposable
    {
        ...
        internal RegistryHive(string path)
        {
            ...
            if (ReadAscii(0, 4) != "regf") throw new InvalidDataException("Not a registry hive");
        }
        ...
        internal int OpenKey(string path)
        {
            ...
            foreach (var part in path.Split(new[] { '\\' }, StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                if (found < 0) throw new KeyNotFoundException($"Registry key not found: {path}");
                cell = found;
            }
            return cell;
        }
        ...
        private IEnumerable<int> EnumerateSubkeys(int keyCell)
        {
            ...
            if (count <= 0 || list < 0) yield break;
            foreach (var cell in ReadSubkeyList(list)) yield return cell;
        }
        ...
        private IEnumerable<int> ReadSubkeyList(int listCell)
        {
            ...
            if (sig == "ri")
            {
                for (var i = 0; i < count; i++) foreach (var child in ReadSubkeyList(ReadInt32(at + 8 + i * 4))) yield return child;
                yield break;
            }
            var stride = sig == "li" ? 4 : sig is "lf" or "lh" ? 8 : throw new InvalidDataException($"Unknown subkey list {sig}");
            for (var i = 0; i < count; i++) yield return ReadInt32(at + 8 + i * stride);
        }
        ...
        private void RequireSignature(int cell, string expected) { if (cell < 0 || ReadAscii(Absolute(cell) + 4, 2) != expected) throw new InvalidDataException($"Invalid {expected} cell"); }
